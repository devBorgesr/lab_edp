"""
exp_e9c — Encarnacao em CODIGO do Experimento E9c (validacao de instrumento, 3a).

Espelha docs/preregistro_experimento_e9c.md. CONGELADO apos o 1o disparo real.

COPIA DELIBERADA DE exp_e9c.py. Difere em DUAS constantes e em nada mais:
EXPERIMENTO e NUM_PREDICT. `tests/test_exp_e9c.py` verifica exatamente isso —
compara o conjunto de constantes dos dois modulos e exige que a diferenca seja
esse par, e compara a fonte da cascata para provar que a copia nao derivou.

POR QUE EXISTE

O E9b foi armado com NUM_PREDICT=64 e a primeira linha de progresso mediu
16,9s/req -> 6,6h. Decomposto:

    prompt_eval   ~1,9s   <- a metrica primaria e ISTO
    eval         ~15,0s   <- 88% do tempo, e nao entra em criterio nenhum

O §8 do E9b exige que a geracao seja CONSTANTE entre condicoes, para
eval_duration nao variar por motivo que nao e o testado. Constante pode ser 1.
Gerar 64 tokens que nenhum criterio le custa 88% da rodada.

NUM_PREDICT nao aparece em NENHUMA regra de decisao (§6). Nao pode virar
veredito. Mas e constante congelada e o disparo do E9b ja tinha comecado —
por isso isto e E9c, nao "E9b corrigido". A regra do §2 e literal.

O QUE ISSO NAO AUTORIZA

Rodada curta = menos deriva termica = controle negativo MAIS FACIL de passar.
A mudanca e favoravel a mim, e por isso a alegacao que o E9c sustenta e mais
estreita: "o instrumento resolve 2x numa rodada de ~45min em maquina
aquietada". Nao "sob quaisquer condicoes". Ver §12.

NAO MEDE ENERGIA. Mesma declaracao do E9 §3.1.

Anti-mock: motor REAL, producao do EDP intocada, harness ocioso durante a
inferencia.
"""
...
TEMPERATURA       = 0
NUM_PREDICT       = 1            # E9c: 64 gerava 88% de trabalho fora da metrica
SEED              = 20260814
N_BOOTSTRAP       = 10000
NIVEL_IC          = 0.95
...
def run_e9c(dry_run: bool = True, saida: Optional[Path] = None) -> ResultadoE9c:
    res = ResultadoE9c(dry_run=dry_run)
...
def score_e9c(amostras: list, exigir_completo: bool = True) -> dict:
    """
...
def _imprimir(res: ResultadoE9c, v: Optional[dict] = None) -> None:
    print("\n" + "=" * 72)
    print(f"E9c — {'DRY-RUN (prova-no-espelho)' if res.dry_run else 'REAL (armado)'}")
    print("=" * 72)
...
def main(argv: Optional[list] = None) -> int:
    p = argparse.ArgumentParser(description="Experimento E9c")
    p.add_argument("--dry-run", action="store_true")
    p.add_argument("--saida", default="e9c_amostras.jsonl")
    ...
    if args.score:
        ...
        r = ResultadoE9c(dry_run=False, armed=True, amostras=am)
        r.digest = next((a.get("digest", "") for a in am if a.get("digest")), "")
        _imprimir(r, score_e9c(am))
        return 0

    try:
        res = run_e9c(dry_run=args.dry_run,
    ...
    v = score_e9c(res.amostras) if not res.dry_run else None
    _imprimir(res, v)
    return 0
